...
async function seedPokemon() {
  ...
  try {
    ...
    for (let i = 0; i < listData.results.length; i += batchSize) {
      ...
      const pokemonDataPromises = batch.map(async (p) => {
        try {
          ...
          console.warn(`Failed to process ${p.name}:`, err.message);
          return null;
        }
      });
      
      const pokemonBatch = (await Promise.all(pokemonDataPromises)).filter(Boolean);
